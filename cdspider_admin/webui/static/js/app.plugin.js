  ...
  $(function(){
    ...
    $('.easypiechart').each(function(){
        ...
        $(this).easyPieChart({
            ...
            onStop: function(){
                var $this = this.$el;
                $this.data("loop") && setTimeout(function(){ $this.data("loop") && $callback($this) }, 10000);
            }
        });
    });
    ...
    $(document).on('click', '[data-toggle^="class:pie"]', function (e) {
    	e && e.preventDefault();
	    var $btn = $(e.target), $loop = $('[data-loop]').data('loop'), $callback=updatePie, $target;
	    ...
	    !$target && ($target = $btn.closest('[data-loop]'));
            ...
            if ($target.data('callback')) {
                var callback = eval($target.data('callback'));
                if (typeof(callback) == 'function') {
                    $callback = callback;
                }
            }
            !$loop && $callback($target);
	});
	...
	// wizard
        $(document).on('changed', $('[data-wizard]:eq(0)').attr('data-target'), function (e) {
            var $activeLi = $('.steps li.active');
            var acname = $activeLi.data('callback')
            if (acname != 'undefined') {
                var ac= eval(acname)
                try{
                    ...
                    console.log( acname +' not function')
                }
            }
        })
	$(document).on('click', '[data-wizard]', function (e) {
	    ...
	    var stepNums = $target.find('.steps li').length;
	    ...
	    var $step = $(this).closest('.step-content').find('.step-pane:eq(' + (item.step-1) + ')');
            $step.find('input, select, textarea').attr('data-parsley-group', 'block-' + (item.step-1));
            var vr = true
            $(this).closest('.step-content').find('form').parsley().whenValidate({
               group: 'block-' + (item.step-1)
            }).fail(function(){
                vr = false
            }).done(function() {
              vr = true
	    ...
	    if(!vr ){
	    	...
	    	prev.attr('disabled', (activeStep == 1) ? true : false);
                ...
                if (option=="next" && next.data('callback') != 'undefined'){
                    var callback_name = next.data('callback')
                    var callback = eval(callback_name)
                    try{
                        if (typeof(callback) == "function") {
                            var ret = callback($step)
                            if (!ret) {
                                return false
                            }
                        }
                    } catch(e) {
                        console.log( callback_name +' not function')
                    }
                }
	    	$target.wizard(option);
                var kids = next.children().detach();
                if (activeStep == stepNums) {
                    if (next.data('last') != 'undefined') {
                        var txt = next.text()
                        console.log(txt)
                        next.text(next.data('last')).append(kids);
                        next.data('next', txt)
                    }
                } else {
                    if (next.data('next') != 'undefined') {
                        next.text(next.data('next')).append(kids);
                    }
                }
	    }
	});
